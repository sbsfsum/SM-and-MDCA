...
class FocalLoss(nn.Module):
  ...
  def forward(self, pred, gt):
      ...
      #pos_loss = torch.log(pred + 1e-20) * torch.pow(1 - pred, 2) * pos_inds
      pos_loss = torch.log2(pred + 1e-20) * torch.pow(1 - pred, 2) * pos_inds
      #neg_loss = torch.log(1 - pred) * torch.pow(pred, 2) * neg_weights * neg_inds
      neg_loss = torch.log2(1 - pred) * torch.pow(pred, 2) * neg_weights * neg_inds
